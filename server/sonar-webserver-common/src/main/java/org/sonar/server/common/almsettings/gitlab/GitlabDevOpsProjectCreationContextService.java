...
@ServerSide
public class GitlabDevOpsProjectCreationContextService
    implements DevOpsProjectCreationContextService {

  private final DbClient dbClient;
  private final UserSession userSession;
  private final GitlabApplicationClient gitlabApplicationClient;

  public GitlabDevOpsProjectCreationContextService(
      DbClient dbClient, UserSession userSession, GitlabApplicationClient gitlabApplicationClient) {
  ...
  @Override
  public DevOpsProjectCreationContext create(
    ...
    return new DevOpsProjectCreationContext(
        gitlabProject.getName(),
        gitlabProject.getPathWithNamespace(),
        String.valueOf(gitlabProjectId),
        gitlabProject.getVisibility().equals("public"),
        defaultBranchName,
        almSettingDto,
        userSession);
  }
          ...
          .orElseThrow(
              () ->
                  new IllegalArgumentException(
                      format("Personal access token for '%s' is missing", almSettingDto.getKey())));
    }
  }

  private Project fetchGitlabProject(String gitlabUrl, String pat, Long gitlabProjectId) {
    try {
      return gitlabApplicationClient.getProject(gitlabUrl, pat, gitlabProjectId);
    } catch (GitlabServerException e) {
      throw new IllegalStateException(
          format(
              "Failed to fetch GitLab project with ID '%s' from '%s'", gitlabProjectId, gitlabUrl),
          e);
    }
  }
}
