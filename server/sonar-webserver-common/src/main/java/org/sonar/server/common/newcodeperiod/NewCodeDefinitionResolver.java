...
public class NewCodeDefinitionResolver {
  ...
  public static final String NEW_CODE_PERIOD_TYPE_DESCRIPTION_PROJECT_CREATION =
      "Project New Code Definition Type<br/>"
  ...
  public static final String NEW_CODE_PERIOD_VALUE_DESCRIPTION_PROJECT_CREATION =
      "Project New Code Definition Value<br/>"
          + "For each new code definition type, a different value is expected:"
          + BEGIN_LIST
          + BEGIN_ITEM_LIST
          + "no value, when the new code definition type is "
          + PREVIOUS_VERSION.name()
          + " and "
          + REFERENCE_BRANCH.name()
          + END_ITEM_LIST
          + BEGIN_ITEM_LIST
          + "a number between 1 and 90, when the new code definition type is "
          + NUMBER_OF_DAYS.name()
          + END_ITEM_LIST
          + END_LIST;

  private static final String UNEXPECTED_VALUE_ERROR_MESSAGE =
      "Unexpected value for newCodeDefinitionType '%s'";

  private static final EnumSet<NewCodePeriodType> projectCreationNCDTypes =
  ...
  public void createNewCodeDefinition(
      DbSession dbSession,
      String projectUuid,
      String mainBranchUuid,
    ...
    getNewCodeDefinitionValueProjectCreation(
            newCodePeriodType, newCodeDefinitionValue, defaultBranchName)
        .ifPresent(dto::setValue);

    if (!CaycUtils.isNewCodePeriodCompliant(dto.getType(), dto.getValue())) {
      throw new IllegalArgumentException(
          "Failed to set the New Code Definition. The given value is not compatible with the Clean"
              + " as You Code methodology. Please refer to the documentation for compliant"
              + " options.");
    }
  ...
  public static void checkNewCodeDefinitionParam(
  ...
  private static Optional<String> getNewCodeDefinitionValueProjectCreation(
      NewCodePeriodType type, @Nullable String value, String defaultBranchName) {
    return switch (type) {
  ...
  private static void requireValue(NewCodePeriodType type, @Nullable String value) {
    Preconditions.checkArgument(
        value != null, "New code definition type '%s' requires a newCodeDefinitionValue", type);
  }
  ...
  private static void validateType(NewCodePeriodType type) {
    Preconditions.checkArgument(
        projectCreationNCDTypes.contains(type),
        "Invalid type '%s'. `newCodeDefinitionType` can only be set with types: %s",
        type,
        projectCreationNCDTypes);
  }
}
