...
public class GitlabConfigurationService {
  ...
  private static final List<String> GITLAB_CONFIGURATION_PROPERTIES =
      List.of(
          GITLAB_AUTH_ENABLED,
  ...
  public GitlabConfigurationService(
      DbClient dbClient,
      ManagedInstanceService managedInstanceService,
      GitlabGlobalSettingsValidator gitlabGlobalSettingsValidator,
  ...
  public GitlabConfiguration updateConfiguration(UpdateGitlabConfigurationRequest updateRequest) {
    UpdatedValue<Boolean> provisioningEnabled =
        updateRequest
            .provisioningType()
            .map(GitlabConfigurationService::shouldEnableAutoProvisioning);
    ...
    try (DbSession dbSession = dbClient.openSession(true)) {
      ...
      GitlabConfiguration currentConfiguration =
          getConfiguration(updateRequest.gitlabConfigurationId(), dbSession);

      ProvisioningType provisioningType =
          updateRequest.provisioningType().orElse(currentConfiguration.provisioningType());
      Set<String> allowedGroups =
          updateRequest.allowedGroups().orElse(currentConfiguration.allowedGroups());
      ...
      setIfDefined(
          dbSession,
          GITLAB_AUTH_SYNC_USER_GROUPS,
          updateRequest.synchronizeGroups().map(String::valueOf));
      setIfDefined(
          dbSession,
          GITLAB_AUTH_ALLOWED_GROUPS,
          updateRequest.allowedGroups().map(groups -> String.join(",", groups)));
      setIfDefined(
          dbSession, GITLAB_AUTH_PROVISIONING_ENABLED, provisioningEnabled.map(String::valueOf));
      setIfDefined(
          dbSession,
          GITLAB_AUTH_ALLOW_USERS_TO_SIGNUP,
      ...
      GitlabConfiguration updatedConfiguration =
          getConfiguration(UNIQUE_GITLAB_CONFIGURATION_ID, dbSession);
  ...
  private static void throwIfUrlIsUpdatedWithoutSecrets(UpdateGitlabConfigurationRequest request) {
    if (request.url().isDefined()) {
      checkArgument(
          request.secret().isDefined(),
          "For security reasons, the URL can't be updated without providing the secret.");
    }
  }
  ...
  private void deleteExternalGroupsWhenDisablingAutoProvisioning(
    ...
    if (disableAutoProvisioning) {
      dbClient
          .externalGroupDao()
          .deleteByExternalIdentityProvider(dbSession, GitLabIdentityProvider.KEY);
    }
  }
  ...
  private String getStringPropertyOrNull(DbSession dbSession, String property) {
    return Optional.ofNullable(dbClient.propertiesDao().selectGlobalProperty(dbSession, property))
        .map(dto -> Strings.emptyToNull(dto.getValue()))
        .orElse(null);
  }
  ...
  public void deleteConfiguration(String id) {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      throwIfConfigurationDoesntExist(dbSession);
      GITLAB_CONFIGURATION_PROPERTIES.forEach(
  ...
  private void throwIfConfigurationDoesntExist(DbSession dbSession) {
    checkFound(
        dbClient.propertiesDao().selectGlobalProperty(dbSession, GITLAB_AUTH_ENABLED),
        "GitLab configuration doesn't exist.");
  }
  ...
  public GitlabConfiguration createConfiguration(GitlabConfiguration configuration) {
    ...
    throwIfAllowedGroupsEmptyAndAutoProvisioning(
        configuration.provisioningType(), configuration.allowedGroups());
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      setProperty(
          dbSession,
          GITLAB_AUTH_SYNC_USER_GROUPS,
          String.valueOf(configuration.synchronizeGroups()));
      setProperty(
          dbSession, GITLAB_AUTH_ALLOWED_GROUPS, String.join(",", configuration.allowedGroups()));
      setProperty(
          dbSession, GITLAB_AUTH_PROVISIONING_ENABLED, String.valueOf(enableAutoProvisioning));
      setProperty(
          dbSession,
          GITLAB_AUTH_ALLOW_USERS_TO_SIGNUP,
  ...
  private void throwIfConfigurationAlreadyExists() {
    Optional.ofNullable(dbClient.propertiesDao().selectGlobalProperty(GITLAB_AUTH_ENABLED))
        .ifPresent(
            property -> {
              throw BadRequestException.create(
                  "GitLab configuration already exists. Only one Gitlab configuration is"
                      + " supported.");
            });
  }

  private static void throwIfAllowedGroupsEmptyAndAutoProvisioning(
      ProvisioningType provisioningType, Set<String> allowedGroups) {
    if (provisioningType == AUTO_PROVISIONING && allowedGroups.isEmpty()) {
      throw new IllegalArgumentException(
          "allowedGroups cannot be empty when Auto-provisioning is enabled.");
    }
  }
  ...
  private Set<String> getAllowedGroups(DbSession dbSession) {
    return Optional.ofNullable(
            dbClient.propertiesDao().selectGlobalProperty(dbSession, GITLAB_AUTH_ALLOWED_GROUPS))
  ...
  private void throwIfConfigIncompleteOrInstanceAlreadyManaged(GitlabConfiguration configuration) {
    ...
    checkState(
        isNotBlank(configuration.provisioningToken()),
        getErrorMessage("Provisioning token must be set"));
  }

  private void checkInstanceNotManagedByAnotherProvider() {
    if (managedInstanceService.isInstanceExternallyManaged()) {
      Optional.of(managedInstanceService.getProviderName())
          .filter(providerName -> !"gitlab".equals(providerName))
          .ifPresent(
              providerName -> {
                throw new IllegalStateException(
                    "It is not possible to synchronize SonarQube using GitLab, as it is already"
                        + " managed by "
                        + providerName
                        + ".");
              });
  ...
  private static GitlabGlobalSettingsValidator.ValidationMode toValidationMode(
      ProvisioningType provisioningType) {
