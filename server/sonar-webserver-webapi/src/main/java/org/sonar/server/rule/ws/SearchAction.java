...
public class SearchAction implements RulesWsAction {
  ...
  private static final Collection<String> DEFAULT_FACETS =
      Set.of(PARAM_LANGUAGES, PARAM_REPOSITORIES, "tags");
  private static final String[] POSSIBLE_FACETS =
      new String[] {
        FACET_LANGUAGES,
        FACET_REPOSITORIES,
        FACET_TAGS,
  ...
  public SearchAction(
      RuleIndex ruleIndex,
      RuleQueryFactory ruleQueryFactory,
      DbClient dbClient,
      RulesResponseFormatter rulesResponseFormatter) {
  ...
  public void define(WebService.NewController controller) {
    WebService.NewAction action =
        controller
            ...
            .setChangelog(
                ...
                new Change(
                    "5.5",
                    "The field 'effortToFixDescription' has been deprecated, use 'gapDescription'"
                        + " instead"),
                new Change(
                    "5.5",
                    "The field 'debtRemFnCoeff' has been deprecated, use 'remFnGapMultiplier'"
                        + " instead"),
                new Change(
                    "5.5",
                    "The field 'defaultDebtRemFnCoeff' has been deprecated, use"
                        + " 'defaultRemFnGapMultiplier' instead"),
                new Change(
                    "5.5",
                    "The field 'debtRemFnOffset' has been deprecated, use 'remFnBaseEffort'"
                        + " instead"),
                new Change(
                    "5.5",
                    "The field 'defaultDebtRemFnOffset' has been deprecated, use"
                        + " 'defaultRemFnBaseEffort' instead"),
                new Change(
                    "5.5",
                    "The field 'debtOverloaded' has been deprecated, use 'remFnOverloaded'"
                        + " instead"),
                ...
                new Change(
                    "9.5",
                    "The field 'htmlDesc' has been deprecated, use 'descriptionSections' instead"),
                ...
                new Change(
                    "10.0",
                    "The deprecated field 'effortToFixDescription' has been removed, use"
                        + " 'gapDescription' instead."),
                new Change(
                    "10.0",
                    "The deprecated field 'debtRemFnCoeff' has been removed, use"
                        + " 'remFnGapMultiplier' instead."),
                new Change(
                    "10.0",
                    "The deprecated field 'defaultDebtRemFnCoeff' has been removed, use"
                        + " 'defaultRemFnGapMultiplier' instead."),
                new Change(
                    "10.0",
                    "The deprecated field 'debtRemFnOffset' has been removed, use 'remFnBaseEffort'"
                        + " instead."),
                new Change(
                    "10.0",
                    "The deprecated field 'defaultDebtRemFnOffset' has been removed, use"
                        + " 'defaultRemFnBaseEffort' instead."),
                new Change(
                    "10.0",
                    "The deprecated field 'debtOverloaded' has been removed, use 'remFnOverloaded'"
                        + " instead."),
                new Change(
                    "10.0",
                    "The field 'defaultDebtRemFnType' has been deprecated, use 'defaultRemFnType'"
                        + " instead"),
                new Change(
                    "10.0",
                    "The field 'debtRemFnType' has been deprecated, use 'remFnType' instead"),
                new Change(
                    "10.0",
                    "The value 'debtRemFn' for the 'f' parameter has been deprecated, use 'remFn'"
                        + " instead"),
                new Change(
                    "10.0",
                    "The value 'defaultDebtRemFn' for the 'f' parameter has been deprecated, use"
                        + " 'defaultRemFn' instead"),
                new Change(
                ...
                new Change(
                    "10.2",
                    "The fields 'type' and 'severity' are deprecated in the response. Use 'impacts'"
                        + " instead."),
                new Change(
                ...
                new Change(
                    "10.2",
                    format(
                        "The values '%s', '%s' and '%s' have been added to the 'facets' parameter.",
                        FACET_CLEAN_CODE_ATTRIBUTE_CATEGORY,
                        FACET_IMPACT_SOFTWARE_QUALITY,
                        FACET_IMPACT_SEVERITY)),
                new Change(
                    "10.2",
                    format(
                        "The values 'severity' and 'types' for the 'facets' parameter have been"
                            + " deprecated. Use '%s' and '%s' instead.",
                        FACET_IMPACT_SEVERITY, FACET_IMPACT_SOFTWARE_QUALITY)),
                new Change(
                    "10.2",
                    format(
                        "Parameters '%s', '%s', and '%s' are now deprecated. Use '%s' and '%s'"
                            + " instead.",
                        PARAM_SEVERITIES,
                        PARAM_TYPES,
    ...
    WebService.NewParam paramFields =
        action
            .createParam(FIELDS)
            .setDescription(
                "Comma-separated list of additional fields to be returned in the response. All the"
                    + " fields are returned by default, except actives.")
    ...
    action
        .setDescription(
            "Search for a collection of relevant rules matching a specified query.<br/>")
  ...
  public void handle(Request request, Response response) throws Exception {
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      SearchResponse responseBuilder =
          buildResponse(dbSession, searchWsRequest, context, searchResult, query);
  ...
  private static void writeStatistics(
      SearchResponse.Builder response, SearchResult searchResult, SearchOptions context) {
    response.setTotal(searchResult.getTotal());
    response.setP(context.getPage());
    response.setPs(context.getLimit());
    response.setPaging(
        formatPaging(searchResult.getTotal(), context.getPage(), context.getLimit()));
  }
  ...
  private static SearchOptions buildSearchOptions(SearchRequest request) {
    ...
    SearchOptions searchOptions =
        new SearchOptions().setLimit(context.getLimit()).setOffset(context.getOffset());
  ...
  private SearchResult doSearch(DbSession dbSession, RuleQuery query, SearchOptions context) {
    ...
    Map<String, RuleDto> rulesByRuleKey =
        Maps.uniqueIndex(dbClient.ruleDao().selectByUuids(dbSession, ruleUuids), RuleDto::getUuid);
  ...
  private void doContextResponse(
      DbSession dbSession,
      SearchRequest request,
      SearchResult result,
    ...
    response.addAllRules(
        rulesResponseFormatter.formatRulesSearch(
            dbSession, result, contextForResponse.getFields()));
    if (contextForResponse.getFields().contains("actives")) {
      Rules.Actives actives =
          rulesResponseFormatter.formatActiveRules(
              dbSession, query.getQProfile(), result.getRules());
  ...
  private static void writeFacets(
      SearchResponse.Builder response,
      SearchRequest request,
      SearchOptions context,
    ...
    addMandatoryFacetValues(
        results, PARAM_IMPACT_SOFTWARE_QUALITIES, enumToStringCollection(SoftwareQuality.values()));
    addMandatoryFacetValues(
        results,
        PARAM_IMPACT_SEVERITIES,
    ...
    facetValuesByFacetKey.put(
        FACET_CLEAN_CODE_ATTRIBUTE_CATEGORY, request.getCleanCodeAttributesCategories());
    ...
    for (String facetName : context.getFacets()) {
      ...
      if (facets != null) {
        ...
        for (Map.Entry<String, Long> facetValue : facets.entrySet()) {
          ...
          facet.addValues(
              value.clear().setVal(facetValue.getKey()).setCount(facetValue.getValue()));
        }
  ...
  private static void addZeroFacetsForSelectedItems(
      Common.Facet.Builder facet,
      @Nullable List<String> requestParams,
  ...
  private static void addMandatoryFacetValues(
      SearchResult results, String facetName, @Nullable Collection<String> mandatoryValues) {
    ...
    if (facetValues != null) {
      Collection<String> valuesToAdd =
          mandatoryValues == null ? Lists.newArrayList() : mandatoryValues;
  ...
  private static class SearchRequest {
    ...
    /**
     * @deprecated SansTop25 report is outdated, it has been completely deprecated in version 10.0
    ...
    public SearchRequest setImpactSoftwareQualities(
    ...
    public SearchRequest setCleanCodeAttributesCategories(
