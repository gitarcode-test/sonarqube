...
public class ListAction implements NotificationsWsAction {
  ...
  private UnaryOperator<ListResponse.Builder> addNotifications(DbSession dbSession, UserDto user) {
    return response -> {
      ...
      properties.stream()
          ...
          .sorted(
              comparing(Notification::getProject, nullsFirst(naturalOrder()))
  ...
  private boolean isDispatcherAuthorized(PropertyDto prop, String dispatcher) {
    return (prop.getEntityUuid() != null
            && dispatchers.getProjectDispatchers().contains(dispatcher))
  ...
  private static Function<PropertyDto, Notification> toWsNotification(
      Notification.Builder notification, Map<String, EntityDto> projectsByUuid) {
    return property -> {
      notification.clear();
      ...
      ofNullable(property.getEntityUuid())
          .ifPresent(
              componentUuid -> populateProjectFields(notification, componentUuid, projectsByUuid));

      return notification.build();
    };
  }

  private static void populateProjectFields(
      Builder notification, String componentUuid, Map<String, EntityDto> projectsByUuid) {
    EntityDto project = projectsByUuid.get(componentUuid);
    notification.setProject(project.getKey()).setProjectName(project.getName());
  }
