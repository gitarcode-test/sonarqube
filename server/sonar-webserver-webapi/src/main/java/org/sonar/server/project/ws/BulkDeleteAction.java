...
public class BulkDeleteAction implements ProjectsWsAction {
  ...
  public void define(WebService.NewController context) {
    String parameterRequiredMessage =
        format(
            "At least one parameter is required among %s, %s and %s",
    ...
    WebService.NewAction action =
        context
            ...
            .setPost(true)
            .setDescription(
                "Delete one or several projects.<br />"
            ...
            .setChangelog(
                ...
                new Change(
                    "9.1",
                    "The parameter '"
                        + PARAM_ANALYZED_BEFORE
                        + "' takes into account the analysis of all branches and pull requests, not"
    ...
    action
        ...
        .setDescription(
            "Filter the projects for which last analysis of any branch is older than the given date"
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      Set<ComponentDto> componentDtos =
          new HashSet<>(
              dbClient
                  .componentDao()
                  .selectByQuery(dbSession, query, forPage(1).andSize(Integer.MAX_VALUE)));
      List<EntityDto> entities =
          dbClient
              .entityDao()
              .selectByKeys(
                  dbSession, componentDtos.stream().map(ComponentDto::getKey).collect(toSet()));
      Set<String> entityUuids = entities.stream().map(EntityDto::getUuid).collect(toSet());
      Map<String, String> mainBranchUuidByEntityUuid =
          dbClient.branchDao().selectMainBranchesByProjectUuids(dbSession, entityUuids).stream()
  ...
  private void callDeleteListeners(
      Map<String, String> mainBranchUuidByEntityUuid, List<EntityDto> entities) {
    Set<DeletedProject> deletedProjects =
        entities.stream()
            .map(
                entity ->
                    new DeletedProject(
                        Project.from(entity), mainBranchUuidByEntityUuid.get(entity.getUuid())))
            .collect(toSet());
  ...
  private static void checkAtLeastOneParameterIsPresent(SearchRequest searchRequest) {
    ...
    checkArgument(
        atLeastOneParameterIsPresent,
        format(
            "At least one parameter among %s, %s and %s must be provided",
            PARAM_ANALYZED_BEFORE, PARAM_PROJECTS, Param.TEXT_QUERY));
  }
