...
public class InstalledAction implements PluginsWsAction {
  ...
  public void define(WebService.NewController controller) {
    WebService.NewAction action =
        controller
            .createAction("installed")
            .setDescription(
                "Get the list of all the plugins installed on the SonarQube instance, sorted by"
                    + " plugin name.<br/>Requires authentication.")
            .setSince("5.2")
            .setChangelog(
                new Change(
                    "10.4",
                    "The response field 'requiredForLanguages' is added for plugins that support"
                        + " it"),
            ...
            .setHandler(this)
            .setResponseExample(
                Resources.getResource(this.getClass(), "example-installed_plugins.json"));

    action
        .createFieldsParam(singleton("category"))
        .setDescription(
            format(
                "Comma-separated list of the additional fields to be returned in response. No"
                    + " additional field is returned by default. Possible values are:<ul><li>%s -"
                    + " category as defined in the Update Center. A connection to the Update Center"
                    + " is needed</li></ul>",
                FIELD_CATEGORY))
        .setSince("5.6");
  ...
  public void handle(Request request, Response response) throws Exception {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      dtosByKey =
          dbClient.pluginDao().selectAll(dbSession).stream()
              .collect(toMap(PluginDto::getKee, Function.identity()));
    }

    List<String> additionalFields = request.paramAsStrings(WebService.Param.FIELDS);
    Map<String, Plugin> updateCenterPlugins =
        (additionalFields == null || additionalFields.isEmpty())
            ? emptyMap()
    ...
    for (ServerPlugin installedPlugin : installedPlugins) {
      ...
      Objects.requireNonNull(
          pluginDto, () -> format("Plugin %s is installed but not in DB", pluginInfo.getKey()));
      Plugin updateCenterPlugin = updateCenterPlugins.get(pluginInfo.getKey());

      pluginList.add(
          buildPluginDetails(installedPlugin, pluginInfo, pluginDto, updateCenterPlugin));
    }

    InstalledPluginsWsResponse wsResponse =
        InstalledPluginsWsResponse.newBuilder().addAllPlugins(pluginList).build();
