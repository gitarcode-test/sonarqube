...
public class SuggestionsAction implements ComponentsWsAction {
  ...
  public SuggestionsAction(
      DbClient dbClient,
      ComponentIndex index,
      FavoriteFinder favoriteFinder,
  ...
  public void define(WebService.NewController context) {
    NewAction action =
        context
            .createAction(ACTION_SUGGESTIONS)
            .setDescription(
                "Internal WS for the top-right search engine. The result will contain component"
                    + " search results, grouped by their qualifiers.<p>Each result contains:"
                    + "<ul><li>the component key</li><li>the component's name"
                    + " (unescaped)</li><li>optionally a display name, which puts emphasis to"
                    + " matching characters (this text contains html tags and parts of the"
                    + " html-escaped name)</li></ul>")
            .setSince("4.2")
            .setInternal(true)
            .setHandler(this)
            ...
            .setChangelog(
                new Change(
                    "10.0",
                    String.format(
                        "The use of 'BRC' as value for parameter '%s' is no longer supported",
                        PARAM_MORE)),
                new Change(
                    "8.4",
                    String.format(
                        "The use of 'DIR', 'FIL','UTS' as values for parameter '%s' is no longer"
                            + " supported",
                        PARAM_MORE)),
                new Change(
                    "7.6",
                    String.format(
                        "The use of 'BRC' as value for parameter '%s' is deprecated", PARAM_MORE)));
    ...
    action
        .createParam(PARAM_MORE)
        .setDescription(
            "Category, for which to display the next "
                + EXTENDED_LIMIT
                + " results (skipping the first "
                + DEFAULT_LIMIT
                + " results)")
        .setPossibleValues(
            stream(SuggestionCategory.values())
                .map(SuggestionCategory::getName)
                .toArray(String[]::new))
        .setSince("6.4");

    action
        .createParam(PARAM_RECENTLY_BROWSED)
        .setDescription(
            "Comma separated list of component keys, that have recently been browsed by the user."
                + " Only the first "
                + MAXIMUM_RECENTLY_BROWSED
  ...
  public void handle(Request wsRequest, Response wsResponse) throws Exception {
    ...
    SuggestionsWsResponse searchWsResponse =
        loadSuggestions(query, skip, limit, recentlyBrowsedKeys, qualifiers);
  ...
  private SuggestionsWsResponse loadSuggestions(
  ...
  /**
   * we are generating suggestions, by using (1) favorites and (2) recently browsed components
   * (without searching in Elasticsearch)
   */
  private SuggestionsWsResponse loadSuggestionsWithoutSearch(
      int skip, int limit, Set<String> recentlyBrowsedKeys, List<String> qualifiers) {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      ListMultimap<String, EntityDto> entityPerQualifier =
          authorizedEntities.stream().collect(MoreCollectors.index(EntityDto::getQualifier));
      if (entityPerQualifier.isEmpty()) {
        return newBuilder().build();
      }

      Set<String> favoriteUuids =
          favorites.stream().map(EntityDto::getUuid).collect(Collectors.toSet());
      Comparator<EntityDto> favoriteComparator =
          Comparator.comparing(c -> favoriteUuids.contains(c.getUuid()) ? -1 : +1);
      Comparator<EntityDto> comparator = favoriteComparator.thenComparing(EntityDto::getName);

      ComponentIndexResults componentsPerQualifiers =
          ComponentIndexResults.newBuilder()
              .setQualifiers(
                  qualifiers.stream()
                      .map(
                          q -> {
                            List<EntityDto> componentsOfThisQualifier = entityPerQualifier.get(q);
                            List<ComponentHit> hits =
                                componentsOfThisQualifier.stream()
                                    .sorted(comparator)
                                    .skip(skip)
                                    .limit(limit)
  ...
  private SuggestionsWsResponse loadSuggestionsWithSearch(
      String query, int skip, int limit, Set<String> recentlyBrowsedKeys, List<String> qualifiers) {
    ...
    Set<String> favoriteKeys =
        favorites.stream().map(EntityDto::getKey).collect(Collectors.toSet());
    SuggestionQuery.Builder queryBuilder =
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      Set<String> favoriteUuids =
          favorites.stream().map(EntityDto::getUuid).collect(Collectors.toSet());
      SuggestionsWsResponse.Builder searchWsResponse =
          buildResponse(
              recentlyBrowsedKeys, favoriteUuids, componentsPerQualifiers, entities, skip + limit);
  ...
  private List<String> getQualifiers(@Nullable String more) {
    Set<String> availableQualifiers =
        resourceTypes.getAll().stream().map(ResourceType::getQualifier).collect(Collectors.toSet());
    if (more == null) {
  ...
  private SuggestionsWsResponse.Builder buildResponse(
      Set<String> recentlyBrowsedKeys,
      Set<String> favoriteUuids,
      ComponentIndexResults componentsPerQualifiers,
    ...
    Map<String, EntityDto> entitiesByUuids =
        entities.stream().collect(Collectors.toMap(EntityDto::getUuid, Function.identity()));
    return toResponse(
        componentsPerQualifiers, recentlyBrowsedKeys, favoriteUuids, entitiesByUuids, coveredItems);
  }
  ...
  private static SuggestionsWsResponse.Builder toResponse(
      ComponentIndexResults componentsPerQualifiers,
      Set<String> recentlyBrowsedKeys,
      Set<String> favoriteUuids,
      Map<String, EntityDto> entitiesByUuids,
  ...
  private static List<Category> toCategories(
      ComponentIndexResults componentsPerQualifiers,
      Set<String> recentlyBrowsedKeys,
      Set<String> favoriteUuids,
      Map<String, EntityDto> entitiesByUuids,
        ...
        .map(
            qualifier -> {
              List<Suggestion> suggestions =
                  qualifier.getHits().stream()
                      .map(
                          hit ->
                              toSuggestion(
                                  hit, recentlyBrowsedKeys, favoriteUuids, entitiesByUuids))
  ...
  /**
   * @return null when the component exists in Elasticsearch but not in database. That occurs when
   *     failed indexing requests are been recovering.
   */
  private static Optional<Suggestion> toSuggestion(
      ComponentHit hit,
      Set<String> recentlyBrowsedKeys,
      Set<String> favoriteUuids,
      Map<String, EntityDto> entitiesByUuids) {
        ...
        .map(
            result ->
                Suggestion.newBuilder()
                    ...
                    .setMatch(
                        hit.getHighlightedText()
                            .orElse(HtmlEscapers.htmlEscaper().escape(result.getName())))
