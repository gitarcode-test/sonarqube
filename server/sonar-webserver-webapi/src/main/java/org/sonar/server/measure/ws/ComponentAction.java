...
public class ComponentAction implements MeasuresWsAction {
  ...
  private static final Set<String> QUALIFIERS_ELIGIBLE_FOR_BEST_VALUE =
      ImmutableSortedSet.of(Qualifiers.FILE, Qualifiers.UNIT_TEST_FILE);
  ...
  public ComponentAction(
      DbClient dbClient, ComponentFinder componentFinder, UserSession userSession) {
  ...
  public void define(WebService.NewController context) {
    WebService.NewAction action =
        context
            .createAction(ACTION_COMPONENT)
            .setDescription(
                "Return component with specified measures.<br>Requires the following permission:"
                    + " 'Browse' on the project of specified component.")
            ...
            .setChangelog(
                ...
                new Change(
                    "10.4",
                    "The use of 'wont_fix_issues' value in 'metricKeys' param is now deprecated."
                        + " Use 'accepted_issues' instead."),
                new Change(
                    "10.4",
                    "Added new accepted value for the 'metricKeys' param: 'accepted_issues'."),
                new Change(
                    "10.1",
                    String.format(
                ...
                new Change(
                    "9.3",
                    "When the new code period is set to 'reference branch', the response field"
                        + " 'date' under the 'period' field has been removed"),
                new Change(
                    "9.3",
                    format(
  ...
  private ComponentWsResponse doHandle(ComponentRequest request) {
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      SnapshotDto analysis =
          dbClient
              .snapshotDao()
              .selectLastAnalysisByRootComponentUuid(dbSession, component.branchUuid())
              .orElse(null);

      List<MetricDto> metrics =
          searchMetrics(dbSession, new HashSet<>(withRemovedMetricAlias(request.getMetricKeys())));
  ...
  public List<MetricDto> searchMetrics(DbSession dbSession, Set<String> metricKeys) {
    ...
    if (metrics.size() < metricKeys.size()) {
      Set<String> foundMetricKeys =
          metrics.stream().map(MetricDto::getKey).collect(Collectors.toSet());
      Set<String> missingMetricKeys =
          metricKeys.stream().filter(m -> !foundMetricKeys.contains(m)).collect(Collectors.toSet());
      throw new NotFoundException(
          format(
              "The following metric keys are not found: %s", String.join(", ", missingMetricKeys)));
    }
  ...
  private List<LiveMeasureDto> searchMeasures(
      DbSession dbSession, ComponentDto component, Collection<MetricDto> metrics) {
    ...
    List<LiveMeasureDto> measures =
        dbClient
            ...
            .selectByComponentUuidsAndMetricUuids(
                dbSession, singletonList(component.uuid()), metricUuids);
    addBestValuesToMeasures(measures, component, metrics);
    return measures;
  }

  private static Map<MetricDto, LiveMeasureDto> getMeasuresByMetric(
      List<LiveMeasureDto> measures, Collection<MetricDto> metrics) {
  ...
  private static void addBestValuesToMeasures(
      List<LiveMeasureDto> measures, ComponentDto component, Collection<MetricDto> metrics) {
    ...
    Map<String, LiveMeasureDto> measuresByMetricUuid =
        Maps.uniqueIndex(measures, LiveMeasureDto::getMetricUuid);
  ...
  private ComponentDto loadComponent(
      DbSession dbSession,
  ...
  private Optional<RefComponent> getReference(DbSession dbSession, ComponentDto component) {
    ...
    Optional<ComponentDto> refComponent =
        dbClient.componentDao().selectByUuid(dbSession, copyComponentUuid);
    if (refComponent.isEmpty()) {
      return Optional.empty();
    }
    Optional<BranchDto> refBranch =
        dbClient.branchDao().selectByUuid(dbSession, refComponent.get().branchUuid());
    return refBranch.map(rb -> new RefComponent(rb, refComponent.get()));
  }

  private ComponentWsResponse buildResponse(
      DbSession dbSession,
      ComponentRequest request,
      ComponentDto component,
      Map<MetricDto, LiveMeasureDto> measuresByMetric,
      Collection<MetricDto> metrics,
      @Nullable Measures.Period period,
    ...
    if (reference != null) {
      ...
      response.setComponent(
          componentDtoToWsComponent(
              component,
              measuresByMetric,
              singletonMap(refComponent.uuid(), refComponent),
              refBranch.isMain() ? null : refBranch.getBranchKey(),
              null,
              requestedMetrics));
    } else {
      boolean isMainBranch =
          dbClient
              .branchDao()
              .selectByUuid(dbSession, component.branchUuid())
  ...
  private static void setAdditionalFields(
      ...
      Collection<MetricDto> metrics,
      @Nullable Measures.Period period,
      ComponentWsResponse.Builder response,
      Collection<String> requestedMetrics) {
      ...
      if (additionalFields.contains(ADDITIONAL_METRICS)) {
        for (MetricDto metricDto : metrics) {
          addMetricToResponseIncludingRenamedMetric(
              metric -> response.getMetricsBuilder().addMetrics(metric),
              requestedMetrics,
              metricDto);
        }
      }
  ...
  private static ComponentRequest toComponentWsRequest(Request request) {
    ...
    checkRequest(
        !componentRequest.getMetricKeys().isEmpty(), "At least one metric key must be provided");
