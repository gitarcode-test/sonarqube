...
public class ComponentAction implements NavigationWsAction {
  ...
  private static final Set<String> MODULE_OR_DIR_QUALIFIERS =
      Set.of(Qualifiers.MODULE, Qualifiers.DIRECTORY);
  ...
  private static final Set<String> QUALIFIERS_WITH_VISIBILITY =
      Set.of(Qualifiers.PROJECT, Qualifiers.VIEW, Qualifiers.APP);
  ...
  public void define(NewController context) {
    NewAction action =
        context
            ...
            .setDescription(
                "Get information concerning component navigation for the current user. Requires the"
                    + " 'Browse' permission on the component's project. <br>For applications, it"
                    + " also requires 'Browse' permission on its child projects.")
            .setHandler(this)
            .setInternal(true)
            ...
            .setChangelog(
                ...
                new Change(
                    "8.8",
                    "Deprecated parameter 'componentKey' has been removed. Please use parameter"
                        + " 'component' instead"),
                new Change(
                    "7.6",
                    format(
  ...
  public void handle(Request request, Response response) throws Exception {
    ...
    try (DbSession session = dbClient.openSession(false)) {
      ...
      ComponentDto component =
          componentFinder.getByKeyAndOptionalBranchOrPullRequest(
              session, componentKey, branch, pullRequest);
      ...
      Optional<BranchDto> branchDto =
          dbClient.branchDao().selectByUuid(session, rootComponent.branchUuid());
      String projectOrPortfolioUuid =
          branchDto.map(BranchDto::getProjectUuid).orElse(rootComponent.branchUuid());
      if (!userSession.hasComponentPermission(USER, component)
          && !userSession.hasComponentPermission(ADMIN, component)
          && !userSession.isSystemAdministrator()) {
        throw insufficientPrivilegesException();
      }
      Optional<SnapshotDto> analysis =
          dbClient
              .snapshotDao()
              .selectLastAnalysisByRootComponentUuid(session, component.branchUuid());
      ...
      try (JsonWriter json = response.newJsonWriter()) {
        ...
        writeComponent(
            ...
            isFavourite,
            branchDto.map(BranchDto::getBranchKey).orElse(null));
  ...
  private static void checkComponentNotAModuleAndNotADirectory(ComponentDto component) {
    BadRequestException.checkRequest(
        !MODULE_OR_DIR_QUALIFIERS.contains(component.qualifier()),
        "Operation not supported for module or directory components");
  }
  ...
  private void writeComponent(
    ...
    if (Qualifiers.APP.equals(component.qualifier())) {
      json.prop(
          "canBrowseAllChildProjects", userSession.hasChildProjectsPermission(USER, component));
    }
    if (Qualifiers.VIEW.equals(component.qualifier())
        || Qualifiers.SUBVIEW.equals(component.qualifier())) {
      json.prop(
          "canBrowseAllChildProjects",
          userSession.hasPortfolioChildProjectsPermission(USER, component));
    }
  ...
  private boolean isFavourite(
      DbSession session, String projectOrPortfolioUuid, ComponentDto component) {
    PropertyQuery propertyQuery =
        PropertyQuery.builder()
            ...
            .build();
    List<PropertyDto> componentFavourites =
        dbClient.propertiesDao().selectByQuery(propertyQuery, session);
    return componentFavourites.size() == 1;
  }

  private static boolean isSubview(ComponentDto component) {
    return Qualifiers.SUBVIEW.equals(component.qualifier())
        && Scopes.PROJECT.equals(component.scope());
  }

  private void writeProfiles(JsonWriter json, DbSession dbSession, ComponentDto component) {
    Set<QualityProfile> qualityProfiles =
        dbClient
            .liveMeasureDao()
            .selectMeasure(dbSession, component.branchUuid(), QUALITY_PROFILES_KEY)
            .map(LiveMeasureDto::getDataAsString)
            .map(data -> QPMeasureData.fromJson(data).getProfiles())
            .orElse(emptySortedSet());
    Map<String, QProfileDto> dtoByQPKey =
        dbClient
            .qualityProfileDao()
            .selectByUuids(
                dbSession, qualityProfiles.stream().map(QualityProfile::getQpKey).toList())
            .stream()
  ...
  private void writeQualityGate(JsonWriter json, DbSession session, String projectOrPortfolioUuid) {
    var qualityGateData =
        qualityGateFinder.getEffectiveQualityGate(session, projectOrPortfolioUuid);
    json.name("qualityGate")
        .beginObject()
  ...
  private void writeConfigPageAccess(
      JsonWriter json, boolean isProjectAdmin, ComponentDto component) {
    boolean isProject = Qualifiers.PROJECT.equals(component.qualifier());
    boolean showBackgroundTasks =
        isProjectAdmin
            && (isProject
                || Qualifiers.VIEW.equals(component.qualifier())
                || Qualifiers.APP.equals(component.qualifier()));
    boolean isQualityProfileAdmin =
        userSession.hasPermission(GlobalPermission.ADMINISTER_QUALITY_PROFILES);
    boolean isQualityGateAdmin =
        userSession.hasPermission(GlobalPermission.ADMINISTER_QUALITY_GATES);
    ...
    boolean allowChangingPermissionsByProjectAdmins =
        config
            .getBoolean(CORE_ALLOW_PERMISSION_MANAGEMENT_FOR_PROJECT_ADMINS_PROPERTY)
            .orElse(CORE_ALLOW_PERMISSION_MANAGEMENT_FOR_PROJECT_ADMINS_DEFAULT_VALUE);

    json.prop(
        "showSettings",
        isProjectAdmin && componentTypeHasProperty(component, PROPERTY_CONFIGURABLE));
    ...
    json.prop(
        "showHistory",
        isProjectAdmin && componentTypeHasProperty(component, PROPERTY_MODIFIABLE_HISTORY));
    json.prop(
        "showUpdateKey",
        isProjectAdmin && componentTypeHasProperty(component, PROPERTY_UPDATABLE_KEY));
