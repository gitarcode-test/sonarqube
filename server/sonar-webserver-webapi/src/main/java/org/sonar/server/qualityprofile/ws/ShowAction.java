...
public class ShowAction implements QProfileWsAction {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      QProfileDto profile =
          qProfileWsSupport.getProfile(
              dbSession, QProfileReference.fromKey(request.mandatoryParam(PARAM_KEY)));
      ...
      long deprecatedActiveRuleCount =
          countActiveRulesByQuery(dbSession, profile, builder.setRuleStatus(DEPRECATED));
  ...
  private long countActiveRulesByQuery(
      DbSession dbSession, QProfileDto profile, ActiveRuleCountQuery.Builder queryBuilder) {
    Map<String, Long> result =
        dbClient
            .activeRuleDao()
            .countActiveRulesByQuery(
                dbSession, queryBuilder.setProfiles(singletonList(profile)).build());
  ...
  private long countProjectsByProfiles(DbSession dbSession, QProfileDto profile) {
    Map<String, Long> projects =
        dbClient.qualityProfileDao().countProjectsByProfiles(dbSession, singletonList(profile));
  ...
  public CompareToSonarWay getSonarWay(Request request, DbSession dbSession, QProfileDto profile) {
    ...
    long missingRuleCount =
        dbClient
            .activeRuleDao()
            .countMissingRules(
                dbSession, profile.getRulesProfileUuid(), sonarWay.getRulesProfileUuid());
  ...
  private static ShowResponse buildResponse(
      QProfileDto profile,
      boolean isDefault,
      Language language,
      long activeRules,
      long deprecatedActiveRules,
      long projects,
    ...
    ofNullable(profile.getUserUpdatedAt())
        .ifPresent(userUpdatedAt -> profileBuilder.setUserUpdatedAt(formatDateTime(userUpdatedAt)));
