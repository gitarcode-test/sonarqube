...
public class MigrateDbActionTest {
  ...
  private static final String MESSAGE_NO_MIGRATION_ON_EMBEDDED_DATABASE =
      "Upgrade is not supported on embedded database.";
  ...
  private MigrateDbAction underTest =
      new MigrateDbAction(databaseVersion, database, migrationState, databaseMigration);
  ...
  @Test
  public void
      msg_is_operational_and_state_from_database_migration_when_databaseversion_status_is_UP_TO_DATE() {
    ...
    assertJson(response.getInput())
        .isSimilarTo(expectedResponse(STATUS_NO_MIGRATION, MESSAGE_STATUS_NONE));
  }
  ...
  public void state_from_database_migration_when_dbmigration_status_is_RUNNING(
    ...
    assertJson(response.getInput())
        .isSimilarTo(expectedResponse(STATUS_MIGRATION_RUNNING, MESSAGE_STATUS_RUNNING, SOME_DATE));
  }
  ...
  public void
    ...
    assertJson(response.getInput())
        .isSimilarTo(
            expectedResponse(STATUS_MIGRATION_FAILED, failedMsg(SOME_THROWABLE_MSG), SOME_DATE));
  }
  ...
  public void
    ...
    assertJson(response.getInput())
        .isSimilarTo(
            expectedResponse(STATUS_MIGRATION_FAILED, failedMsg(DEFAULT_ERROR_MSG), SOME_DATE));
  }
  ...
  private static String expectedResponse(String status, String msg, Instant date) {
    return "{"
        ...
        + "\","
        + "\"startedAt\":\""
        + DATE_TIME_FORMATTER.format(date.atZone(ZoneOffset.UTC))
        + "\""
        + "}";
  }
}
