...
class DBSessionsImplTest {
  ...
  void openSession_without_caching_always_returns_a_new_regular_session_when_parameter_is_false() {
    DbSession[] expected = {
      mock(DbSession.class), mock(DbSession.class), mock(DbSession.class), mock(DbSession.class)
    };
  ...
  void openSession_with_caching_always_returns_the_same_regular_session_when_parameter_is_false() {
    ...
    when(myBatis.openSession(false)).thenReturn(expected).thenThrow(oneCallTooMuch());
    underTest.enableCaching();

    int size = 1 + abs(random.nextInt(10));
    Set<DbSession> dbSessions =
        IntStream.range(0, size)
            .mapToObj(ignored -> underTest.openSession(false))
  ...
  void openSession_with_caching_always_returns_the_same_batch_session_when_parameter_is_true() {
    DbSession expected = mock(DbSession.class);
    when(myBatis.openSession(true)).thenReturn(expected).thenThrow(oneCallTooMuch());
    underTest.enableCaching();

    int size = 1 + abs(random.nextInt(10));
    Set<DbSession> dbSessions =
        IntStream.range(0, size)
            .mapToObj(ignored -> underTest.openSession(true))
            .collect(Collectors.toSet());
    assertThat(dbSessions).hasSize(size);
    assertThat(getWrappedDbSessions(dbSessions)).hasSize(1).containsOnly(expected);
  }
  ...
  void openSession_with_caching_returns_a_session_per_thread() {
    ...
    Thread[] threads = {
      new Thread(runnable, "T1"), new Thread(runnable, "T2"), new Thread(runnable, "T3")
    };
  ...
  @Test
  void
      openSession_with_caching_returns_wrapper_of_MyBatis_DbSession_which_delegates_all_methods_but_close() {
    boolean batchOrRegular = random.nextBoolean();

    underTest.enableCaching();

    verifyFirstDelegation(
        batchOrRegular,
        (myBatisDbSession, dbSession) -> {
          dbSession.rollback();
  ...
  @Test
  void
      openSession_with_caching_returns_DbSession_that_rolls_back_on_close_if_any_mutation_call_was_not_followed_by_commit_nor_rollback()
          throws SQLException {
  ...
  @Test
  void
      openSession_with_caching_returns_DbSession_that_does_not_roll_back_on_close_if_any_mutation_call_was_followed_by_commit()
          throws SQLException {
    DbSession dbSession = openSessionAndDoSeveralMutatingAndNeutralCalls();
  ...
  @Test
  void
      openSession_with_caching_returns_DbSession_that_does_not_roll_back_on_close_if_any_mutation_call_was_followed_by_rollback_without_parameters()
          throws SQLException {
    DbSession dbSession = openSessionAndDoSeveralMutatingAndNeutralCalls();
    dbSession.rollback();
  ...
  @Test
  void
      openSession_with_caching_returns_DbSession_that_does_not_roll_back_on_close_if_any_mutation_call_was_followed_by_rollback_with_parameters()
          throws SQLException {
  ...
  private DbSession openSessionAndDoSeveralMutatingAndNeutralCalls() throws SQLException {
    ...
    int[] dirtyCallIndices =
        IntStream.range(0, dirtyingCallsCount)
            .map(ignored -> abs(random.nextInt(DIRTYING_CALLS.length)))
            .toArray();
    int[] neutralCallsIndices =
        IntStream.range(0, neutralCallsCount)
            .map(ignored -> abs(random.nextInt(NEUTRAL_CALLS.length)))
            .toArray();
  ...
  private static DbSessionCaller[] COMMIT_CALLS = {
    session -> session.commit(), session -> session.commit(new Random().nextBoolean()),
  };

  private static DbSessionCaller[] ROLLBACK_CALLS = {
    session -> session.rollback(), session -> session.rollback(new Random().nextBoolean()),
  };
  ...
  private static DbSessionCaller[] NEUTRAL_CALLS = {
    ...
    session ->
        session.select(
            randomAlphabetic(3), new Object(), new RowBounds(), mock(ResultHandler.class)),
    ...
    session ->
        session.selectMap(randomAlphabetic(3), new Object(), randomAlphabetic(3), new RowBounds()),
  ...
  void disableCaching_does_not_open_DB_connection_if_openSession_was_never_called() {
    when(myBatis.openSession(anyBoolean())).thenThrow(oneCallTooMuch());
    underTest.enableCaching();
  ...
  @Test
  void
      disableCaching_does_not_fail_but_logs_if_closing_MyBatis_session_close_throws_an_exception() {
    boolean batchOrRegular = random.nextBoolean();
    IllegalStateException toBeThrown =
        new IllegalStateException("Faking MyBatisSession#close failing");
  ...
  private void verifyDelegation(
      boolean batchOrRegular, boolean firstCall, BiConsumer<DbSession, DbSession> r) {
    when(myBatis.openSession(batchOrRegular))
        .thenReturn(myBatisDbSession)
        .thenThrow(oneCallTooMuch());
