...
public class QualityProfileRuleChangeResolver {
  ...
  /**
   * Returns a text description of the changes made to a quality profile. The text is generated by
   * looking at the last change for each rule and determining the final status of the rule. For old
   * taxonomy, we need to access the QProfileChangeDto.data field to determine the ruleUuid. For
   * CCT, we can use the QProfileChangeDto.ruleChange field to determine the ruleUuid.
   *
   ...
   */
  public Map<ActiveRuleChange.Type, Long> mapChangeToNumberOfRules(
      QualityProfile profile, String componentUuid) {
    // get profile changes
    List<QProfileChangeDto> profileChanges = getProfileChanges(profile, componentUuid);

    Map<String, List<QProfileChangeDto>> updatedRulesGrouped =
        profileChanges.stream()
            .filter(QualityProfileRuleChangeResolver::hasRuleUuid)
            .collect(
                Collectors.groupingBy(
                    p ->
                        p.getRuleChange() != null
                            ? p.getRuleChange().getRuleUuid()
                            : p.getDataAsMap().get("ruleUuid")));

    Map<String, ActiveRuleChange.Type> rulesMappedToFinalChange =
        getRulesMappedToFinalChange(updatedRulesGrouped);
    return getChangeMappedToNumberOfRules(rulesMappedToFinalChange);
  }

  @NotNull
  private static Map<ActiveRuleChange.Type, Long> getChangeMappedToNumberOfRules(
      Map<String, ActiveRuleChange.Type> rulesMappedToFinalChange) {
  ...
  /**
   * Returns a map of ruleUuid to the final status of the rule. If the rule final status is the same
   * as the initial status, the value will be empty.
   *
   * @param updatedRulesGrouped a map of ruleUuid to a list of changes for that rule
   * @return a map of ruleUuid to the final status of the rule. If the rule final status is the same
   *     as the initial status, the value will be empty.
   */
  private static Map<String, ActiveRuleChange.Type> getRulesMappedToFinalChange(
      Map<String, List<QProfileChangeDto>> updatedRulesGrouped) {
    return updatedRulesGrouped.entrySet().stream()
        .map(
            entry -> {
              ...
              // get last change
              QProfileChangeDto lastChange =
                  ruleChanges.stream()
                      .max(Comparator.comparing(QProfileChangeDto::getCreatedAt))
                      .orElseThrow();
              ...
              if (UPDATED.name().equals(lastChange.getChangeType())) {
                ...
                // for ACTIVATED/DEACTIVATED we need to count the number of times the rule was
                // toggled
                long activationToggles =
                    ruleChanges.stream()
                        .filter(
                            rule ->
                                List.of(ACTIVATED.name(), DEACTIVATED.name())
                                    .contains(rule.getChangeType()))
                        .count();
                ...
                value =
                    activationToggles % 2 == 0
                        ? Optional.empty()
                        : Optional.of(ActiveRuleChange.Type.valueOf(lastChange.getChangeType()));
              }
  ...
  private List<QProfileChangeDto> getProfileChanges(QualityProfile profile, String componentUuid) {
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      List<QProfileChangeDto> profileChanges =
          dbClient.qProfileChangeDao().selectByQuery(dbSession, query);
  ...
  private Long getLastAnalysisDate(String componentUuid, DbSession dbSession) {
    return dbClient
        .snapshotDao()
        .selectLastAnalysisByComponentUuid(dbSession, componentUuid)
