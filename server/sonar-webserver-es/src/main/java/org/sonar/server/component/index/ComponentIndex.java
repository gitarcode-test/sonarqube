...
public class ComponentIndex {
  ...
  public SearchIdResult<String> search(ComponentQuery query, SearchOptions searchOptions) {
    ...
    setNullable(
        query.getQuery(),
        q -> {
          ComponentTextSearchQuery componentTextSearchQuery =
              ComponentTextSearchQuery.builder()
                  .setQueryText(q)
                  .setFieldKey(FIELD_KEY)
                  .setFieldName(FIELD_NAME)
                  .build();
          esQuery.must(
              ComponentTextSearchQueryFactory.createQuery(
                  componentTextSearchQuery, ComponentTextSearchFeatureRepertoire.values()));
        });
    ...
    return new SearchIdResult<>(
        client.search(request), id -> id, system2.getDefaultTimeZone().toZoneId());
  }
  ...
  @VisibleForTesting
  ComponentIndexResults searchSuggestions(
      SuggestionQuery query, ComponentTextSearchFeature... features) {
  ...
  private static FiltersAggregationBuilder createAggregation(SuggestionQuery query) {
    return AggregationBuilders.filters(
            ...
            query.getQualifiers().stream()
                .map(q -> new KeyedFilter(q, termQuery(FIELD_QUALIFIER, q)))
  ...
  private QueryBuilder createQuery(SuggestionQuery query, ComponentTextSearchFeature... features) {
    ...
    ComponentTextSearchQuery componentTextSearchQuery =
        ComponentTextSearchQuery.builder()
            ...
            .build();
    return esQuery.must(
        ComponentTextSearchQueryFactory.createQuery(componentTextSearchQuery, features));
  }
  ...
  private static ComponentIndexResults aggregationsToQualifiers(SearchResponse response) {
    ...
    List<ParsedFilters.ParsedBucket> buckets =
        (List<ParsedFilters.ParsedBucket>) filtersAgg.getBuckets();
    return ComponentIndexResults.newBuilder()
        .setQualifiers(buckets.stream().map(ComponentIndex::bucketToQualifier))
        .build();
  }
  ...
  private static ComponentHitsPerQualifier bucketToQualifier(ParsedFilters.ParsedBucket bucket) {
    ...
    return new ComponentHitsPerQualifier(
        bucket.getKey(),
        ComponentHit.fromSearchHits(hits),
        getTotalHits(hitList.getTotalHits()).value);
  }
