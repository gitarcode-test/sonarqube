...
public class LegacyHotspotRuleDescriptionSectionsGenerator
    implements RuleDescriptionSectionsGenerator {
  ...
  public boolean isGeneratorForRule(RulesDefinition.Rule rule) {
    // To prevent compatibility issues with SonarLint, this Generator is used for all hotspots
    // rules, regardless of if they expose advanced sections or not. See SONAR-16635.
    // In the future, the generator should not be used for advanced rules (add condition &&
    // rule.ruleDescriptionSections().isEmpty())
    return SECURITY_HOTSPOT.equals(rule.type());
  }

  @Override
  public Set<RuleDescriptionSectionDto> generateSections(RulesDefinition.Rule rule) {
    return getDescriptionInHtml(rule).map(this::generateSections).orElse(emptySet());
  }
