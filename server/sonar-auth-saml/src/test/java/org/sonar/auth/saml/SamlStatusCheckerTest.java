...
""";
  public static final String BASE64_ENCRYPTED_SAML_RESPONSE =
      new String(
          Base64.getEncoder().encode(ENCRYPTED_SAML_RESPONSE.getBytes()), StandardCharsets.UTF_8);
  public static final String BASE64_SAML_RESPONSE =
      new String(
          Base64.getEncoder().encode(PLAIN_SAML_RESPONSE.getBytes()), StandardCharsets.UTF_8);

  private final MapSettings settings =
      new MapSettings(new PropertyDefinitions(System2.INSTANCE, SamlSettings.definitions()));
  ...
  public void authentication_status_is_success_when_no_errors() {
    ...
    samlAuthenticationStatus =
        getSamlAuthenticationStatus(
            BASE64_SAML_RESPONSE, auth, new SamlSettings(settings.asConfig()));
  ...
  public void authentication_status_is_unsuccessful_when_errors_are_reported() {
    ...
    assertTrue(
        samlAuthenticationStatus
            .getErrors()
            .contains("Authentication failed due to a missing parameter."));
  ...
  public void authentication_status_is_unsuccessful_when_processResponse_throws_exception() {
    ...
    assertTrue(
        samlAuthenticationStatus.getErrors().contains("Exception when processing the response"));
  }
            ...
            .contains(
                String.format(
                    "Mapping not found for the property %s, the field %s is not available in the"
                        + " SAML response.",
                    GROUP_NAME_ATTRIBUTE, "wrongGroupField")));
    assertTrue(
        samlAuthenticationStatus
            .getWarnings()
            .contains(
                String.format(
                    "Mapping not found for the property %s, the field %s is not available in the"
                        + " SAML response.",
                    USER_EMAIL_ATTRIBUTE, "wrongEmailField")));
  }
        ...
        samlAuthenticationStatus
            ...
            .contains(
                String.format(
                    "Mapping not found for the property %s, the field %s is not available in the"
                        + " SAML response.",
                    USER_LOGIN_ATTRIBUTE, "wrongLoginField")));
    assertTrue(
        samlAuthenticationStatus
            .getErrors()
            .contains(
                String.format(
                    "Mapping not found for the property %s, the field %s is not available in the"
                        + " SAML response.",
                    USER_NAME_ATTRIBUTE, "wrongNameField")));
  }
  ...
  public void authentication_has_errors_when_login_and_name_are_empty() {
    ...
    assertTrue(
        samlAuthenticationStatus
            ...
            .contains(
                String.format(
                    "Mapping found for the property %s, but the field %s is empty in the SAML"
                        + " response.",
  ...
  public void authentication_has_warnings_when_the_private_key_is_invalid_but_auth_completes() {
    ...
    assertTrue(
        samlAuthenticationStatus
            ...
            .contains(
                String.format(
                    "Error in parsing service provider private key, please make sure that it is in"
                        + " PKCS 8 format.")));
  }
  ...
  public void mapped_attributes_are_complete_when_mapping_fields_are_correct() {
    ...
    assertTrue(
        samlAuthenticationStatus
            ...
            .keySet()
            .containsAll(Set.of("login", "name", "email", "groups")));
    assertTrue(
        samlAuthenticationStatus
            .getMappedAttributes()
            .keySet()
            .containsAll(
                Set.of("User login value", "User name value", "User email value", "Groups value")));
  }
  ...
  public void givenEncryptionEnabled_whenUserIsAuthenticated_thenSamlStatusReportsItEnabled() {
    ...
    samlAuthenticationStatus =
        getSamlAuthenticationStatus(
            BASE64_ENCRYPTED_SAML_RESPONSE, auth, new SamlSettings(settings.asConfig()));
  ...
  public void whenSamlResponseIsNull_thenEncryptionIsReportedDisabled() {
    ...
    samlAuthenticationStatus =
        getSamlAuthenticationStatus(null, auth, new SamlSettings(settings.asConfig()));
  ...
  private void setSettings() {
    ...
    settings.setProperty(
        "sonar.auth.saml.providerId", "http://localhost:8080/auth/realms/sonarqube");
    settings.setProperty(
        "sonar.auth.saml.loginUrl", "http://localhost:8080/auth/realms/sonarqube/protocol/saml");
