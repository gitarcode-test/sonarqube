...
public class VisibilityService {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      if (isPrivate != entityDto.isPrivate()) {
        ...
        indexers.commitAndIndexEntities(
            dbSession, singletonList(entityDto), Indexers.EntityEvent.PERMISSION_CHANGE);
      }
    }
  }
  ...
  void checkNoPendingTasks(DbSession dbSession, EntityDto entityDto) {
    // This check likely can be removed when we remove the column 'private' from components table in
    // SONAR-20126.
    checkState(
        countPendingTask(dbSession, entityDto.getKey()) == 0,
        "Component visibility can't be changed as long as it has background task(s) pending or in"
            + " progress");
  }
  ...
  private long countPendingTask(DbSession dbSession, String entityKey) {
    EntityDto entityDto =
        dbClient
            ...
            .orElseThrow(() -> new IllegalStateException("Can't find entity " + entityKey));
    return dbClient.ceQueueDao().selectByEntityUuid(dbSession, entityDto.getUuid()).stream()
        .filter(task -> !task.getTaskType().equals(GITHUB_PROJECT_PERMISSIONS_PROVISIONING))
        .count();
  }

  private void setPrivateForRootComponentUuid(
      DbSession dbSession, EntityDto entity, boolean newIsPrivate) {
    ...
    if (entity.isProjectOrApp()) {
      ...
      dbClient
          .portfolioDao()
          .updateVisibilityByPortfolioUuid(dbSession, entity.getUuid(), newIsPrivate);
    }
  ...
  private void updatePermissionsToPrivate(DbSession dbSession, EntityDto entity) {
    ...
    // grant UserRole.CODEVIEWER and UserRole.USER to any group or user with at least one permission
    // on project
    PUBLIC_PERMISSIONS.forEach(
        permission -> {
          dbClient
              .groupPermissionDao()
              .selectGroupUuidsWithPermissionOnEntityBut(dbSession, entity.getUuid(), permission)
              .forEach(
                  group -> insertProjectPermissionOnGroup(dbSession, entity, permission, group));
          dbClient
              .userPermissionDao()
              .selectUserIdsWithPermissionOnEntityBut(dbSession, entity.getUuid(), permission)
              .forEach(
                  userUuid ->
                      insertProjectPermissionOnUser(dbSession, entity, permission, userUuid));
        });
  }

  private void insertProjectPermissionOnUser(
      DbSession dbSession, EntityDto entity, String permission, UserId userId) {
    dbClient
        .userPermissionDao()
        .insert(
            dbSession,
            new UserPermissionDto(Uuids.create(), permission, userId.getUuid(), entity.getUuid()),
            entity,
            userId,
            null);
  }
  ...
  private void insertProjectPermissionOnGroup(
      DbSession dbSession, EntityDto entity, String permission, String groupUuid) {
    String groupName =
        ofNullable(dbClient.groupDao().selectByUuid(dbSession, groupUuid))
  ...
  private void updatePermissionsToPublic(DbSession dbSession, EntityDto entity) {
    PUBLIC_PERMISSIONS.forEach(
        permission -> {
          ...
          dbClient
              .userPermissionDao()
              .deleteEntityPermissionOfAnyUser(dbSession, permission, entity);
        });
  }
}
