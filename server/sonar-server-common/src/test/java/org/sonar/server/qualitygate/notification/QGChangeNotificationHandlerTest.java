...
public class QGChangeNotificationHandlerTest {
  ...
  private QGChangeNotificationHandler underTest =
      new QGChangeNotificationHandler(notificationManager, emailNotificationChannel);
  ...
  public void deliver_has_no_effect_if_emailNotificationChannel_is_disabled() {
    ...
    Set<QGChangeNotification> notifications =
  ...
  @Test
  public void
      deliver_has_no_effect_if_no_notification_has_subscribed_recipients_to_QGChange_notifications() {
    ...
    when(notificationManager.findSubscribedEmailRecipients(
            QG_CHANGE_DISPATCHER_KEY, projectKey, ALL_MUST_HAVE_ROLE_USER))
        .thenReturn(emptySet());
  ...
  public void deliver_ignores_notification_without_projectKey() {
    ...
    Set<QGChangeNotification> withProjectKey =
        IntStream.range(0, 1 + new Random().nextInt(5))
            .mapToObj(i -> newNotification(projectKey))
            .collect(toSet());
    Set<QGChangeNotification> noProjectKey =
        IntStream.range(0, 1 + new Random().nextInt(5))
            .mapToObj(i -> newNotification(null))
            .collect(toSet());
    Set<NotificationManager.EmailRecipient> emailRecipients =
        IntStream.range(0, 1 + new Random().nextInt(10))
            .mapToObj(i -> "user_" + i)
  ...
  @Test
  public void
      deliver_checks_by_projectKey_if_notifications_have_subscribed_assignee_to_QGChange_notifications() {
    ...
    Set<NotificationManager.EmailRecipient> emailRecipients1 =
        IntStream.range(0, 1 + new Random().nextInt(10))
            .mapToObj(i -> "user1_" + i)
            .map(login -> new NotificationManager.EmailRecipient(login, emailOf(login)))
            .collect(toSet());
    Set<NotificationManager.EmailRecipient> emailRecipients2 =
        IntStream.range(0, 1 + new Random().nextInt(10))
            .mapToObj(i -> "user2_" + i)
            .map(login -> new NotificationManager.EmailRecipient(login, emailOf(login)))
            .collect(toSet());
    ...
    when(notificationManager.findSubscribedEmailRecipients(
            QG_CHANGE_DISPATCHER_KEY, projectKey2, ALL_MUST_HAVE_ROLE_USER))
