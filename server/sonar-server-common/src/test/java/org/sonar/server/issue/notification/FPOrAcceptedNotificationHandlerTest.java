...
public class FPOrAcceptedNotificationHandlerTest {
  ...
  private IssuesChangesNotificationSerializer serializerMock =
      mock(IssuesChangesNotificationSerializer.class);
  private IssuesChangesNotificationSerializer serializer =
      spy(new IssuesChangesNotificationSerializer());
  private Class<Set<EmailDeliveryRequest>> requestSetType =
      (Class<Set<EmailDeliveryRequest>>) (Class<?>) Set.class;
  private FPOrAcceptedNotificationHandler underTest =
      new FPOrAcceptedNotificationHandler(
  ...
  public void deliver_has_no_effect_if_emailNotificationChannel_is_disabled() {
    ...
    Set<IssuesChangesNotification> notifications =
        IntStream.range(0, 5).mapToObj(i -> mock(IssuesChangesNotification.class)).collect(toSet());
  ...
  public void deliver_parses_every_notification_in_order() {
    ...
    FPOrAcceptedNotificationHandler underTest =
        new FPOrAcceptedNotificationHandler(
            notificationManager, emailNotificationChannel, serializerMock);
  ...
  public void deliver_fails_with_IAE_if_serializer_throws_IAE() {
    ...
    IllegalArgumentException expected =
        new IllegalArgumentException("faking serializer#from throwing a IllegalArgumentException");
    when(serializerMock.from(any(IssuesChangesNotification.class)))
        .thenReturn(mock(IssuesChangesNotificationBuilder.class))
        .thenReturn(mock(IssuesChangesNotificationBuilder.class))
        .thenThrow(expected);
  ...
  public void deliver_has_no_effect_if_no_issue_has_FP_or_wontfix_resolution(
            ...
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
                        streamOfIssues(
                                t ->
                                    t.setNewIssueStatus(newIssueStatus)
                                        .setOldIssueStatus(IssueStatus.OPEN))
                            .collect(toSet()),
                        changeMock))
  ...
  public void deliver_shouldNotSendNotification_WhenIssueStatusHasNotChanged(
      String newResolution, IssueStatus newIssueStatus) {
    when(emailNotificationChannel.isActivated()).thenReturn(true);
    Change changeMock = mock(Change.class);
    Set<IssuesChangesNotification> notifications =
        IntStream.range(0, 5)
                ...
                j ->
                    new IssuesChangesNotificationBuilder(
                        streamOfIssues(
                                t ->
                                    t.setNewIssueStatus(newIssueStatus)
                                        .setOldIssueStatus(newIssueStatus))
                            .collect(toSet()),
                        changeMock))
            .map(serializer::serialize)
            .collect(toSet());
  ...
  public static Object[][] notFPorAcceptedIssueStatus() {
    return new Object[][] {{null}, {IssueStatus.FIXED}, {IssueStatus.OPEN}};
  }

  @Test
  @UseDataProvider("FPorWontFixResolutionWithCorrespondingIssueStatus")
  public void
      deliver_checks_by_projectKey_if_notifications_have_subscribed_assignee_to_FPorWontFix_notifications(
          String newResolution, IssueStatus newIssueStatus) {
    ...
    Stream<IssuesChangesNotificationBuilder> project1Notifications =
        IntStream.range(0, 5)
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
                        streamOfIssues(
                                t ->
                                    t.setProject(projectKey1)
                                        .setNewIssueStatus(newIssueStatus)
                                        .setOldIssueStatus(IssueStatus.OPEN))
                            .collect(toSet()),
                        changeMock));
    // some notifications with some issues on project2
    Stream<IssuesChangesNotificationBuilder> project2Notifications =
        IntStream.range(0, 5)
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
                ...
                j ->
                    new IssuesChangesNotificationBuilder(
                        Stream.concat(
                                streamOfIssues(
                                    t ->
                                        t.setProject(projectKey3)
                                            .setNewIssueStatus(newIssueStatus)
                                            .setOldIssueStatus(IssueStatus.OPEN)),
                                streamOfIssues(
                                    t ->
                                        t.setProject(projectKey4)
                                            .setNewIssueStatus(newIssueStatus)
                                            .setOldIssueStatus(IssueStatus.OPEN)))
                            .collect(toSet()),
                        changeMock));
    ...
    Set<IssuesChangesNotification> notifications =
        Stream.of(project1Notifications, project2Notifications, project3And4Notifications)
            .flatMap(t -> t)
            .map(serializer::serialize)
            .collect(toSet());
    int deliver = underTest.deliver(notifications);
    ...
    verify(notificationManager)
        .findSubscribedEmailRecipients(
            DO_NOT_FIX_ISSUE_CHANGE_DISPATCHER_KEY, projectKey1.getKey(), ALL_MUST_HAVE_ROLE_USER);
    verify(notificationManager)
        .findSubscribedEmailRecipients(
            DO_NOT_FIX_ISSUE_CHANGE_DISPATCHER_KEY, projectKey2.getKey(), ALL_MUST_HAVE_ROLE_USER);
    verify(notificationManager)
        .findSubscribedEmailRecipients(
            DO_NOT_FIX_ISSUE_CHANGE_DISPATCHER_KEY, projectKey3.getKey(), ALL_MUST_HAVE_ROLE_USER);
    verify(notificationManager)
        .findSubscribedEmailRecipients(
            DO_NOT_FIX_ISSUE_CHANGE_DISPATCHER_KEY, projectKey4.getKey(), ALL_MUST_HAVE_ROLE_USER);
  ...
  @UseDataProvider("FPorWontFixResolutionWithCorrespondingIssueStatus")
  public void
      deliver_does_not_send_email_request_for_notifications_a_subscriber_is_the_changeAuthor_of(
          String newResolution, IssueStatus newIssueStatus) {
    ...
    // subscriber1 is the changeAuthor of some notifications with issues assigned to subscriber1
    // only
    Set<IssuesChangesNotificationBuilder> subscriber1Notifications =
        IntStream.range(0, 5)
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
                        streamOfIssues(
                                t ->
                                    t.setProject(project)
                                        .setNewIssueStatus(newIssueStatus)
                                        .setOldIssueStatus(IssueStatus.OPEN)
                                        .setAssignee(subscriber2))
                            .collect(toSet()),
                        newUserChange(subscriber1)))
            .collect(toSet());
    // subscriber1 is the changeAuthor of some notifications with issues assigned to subscriber1 and
    // subscriber2
    Set<IssuesChangesNotificationBuilder> subscriber1and2Notifications =
        IntStream.range(0, 5)
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
                                ...
                                streamOfIssues(
                                    t ->
                                        t.setProject(project)
                                            .setNewIssueStatus(newIssueStatus)
                                            .setOldIssueStatus(IssueStatus.OPEN)
                                            .setAssignee(subscriber1)))
                            .collect(toSet()),
                        newUserChange(subscriber1)))
            .collect(toSet());
    // subscriber2 is the changeAuthor of some notifications with issues assigned to subscriber2
    // only
    Set<IssuesChangesNotificationBuilder> subscriber2Notifications =
        IntStream.range(0, 5)
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
            ...
            .collect(toSet());
    // subscriber2 is the changeAuthor of some notifications with issues assigned to subscriber2 and
    // subscriber 3
    Set<IssuesChangesNotificationBuilder> subscriber2And3Notifications =
        IntStream.range(0, 5)
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
                                ...
                                streamOfIssues(
                                    t ->
                                        t.setProject(project)
                                            .setNewIssueStatus(newIssueStatus)
                                            .setOldIssueStatus(IssueStatus.OPEN)
                                            .setAssignee(subscriber3)))
                            .collect(toSet()),
                        newUserChange(subscriber2)))
            .collect(toSet());
    // subscriber3 is the changeAuthor of no notification
    // otherChangeAuthor has some notifications
    Set<IssuesChangesNotificationBuilder> otherChangeAuthorNotifications =
        IntStream.range(0, 5)
            .mapToObj(
                j ->
                    new IssuesChangesNotificationBuilder(
    ...
    Set<String> subscriberLogins =
        ImmutableSet.of(subscriber1.getLogin(), subscriber2.getLogin(), subscriber3.getLogin());
    when(notificationManager.findSubscribedEmailRecipients(
            DO_NOT_FIX_ISSUE_CHANGE_DISPATCHER_KEY, project.getKey(), ALL_MUST_HAVE_ROLE_USER))
        .thenReturn(
            subscriberLogins.stream()
                .map(FPOrAcceptedNotificationHandlerTest::emailRecipientOf)
                .collect(toSet()));
    ...
    verifyNoMoreInteractions(emailNotificationChannel);
    ListMultimap<String, EmailDeliveryRequest> requestsByRecipientEmail =
        captor.getValue().stream().collect(index(EmailDeliveryRequest::recipientEmail));
        ...
        .containsOnly(
            Stream.of(
                    subscriber2Notifications.stream()
                        .map(
                            notif ->
                                newEmailDeliveryRequest(
                                    notif, subscriber1, toFpOrAccepted(newResolution))),
                    subscriber2And3Notifications.stream()
                        .map(
                            notif ->
                                newEmailDeliveryRequest(
            ...
            Stream.of(
                    subscriber1Notifications.stream()
                        .map(
                            notif ->
                                newEmailDeliveryRequest(
                                    notif, subscriber2, toFpOrAccepted(newResolution))),
                    subscriber1and2Notifications.stream()
                        .map(
                            notif ->
                                newEmailDeliveryRequest(
            ...
            Stream.of(
                    subscriber1Notifications.stream()
                        .map(
                            notif ->
                                newEmailDeliveryRequest(
                                    notif, subscriber3, toFpOrAccepted(newResolution))),
                    subscriber1and2Notifications.stream()
                        .map(
                            notif ->
                                newEmailDeliveryRequest(
  ...
  public void deliver_send_a_separated_email_request_for_FPs_and_Wont_Fix_issues(int projectCount) {
    Set<Project> projects =
        IntStream.range(0, projectCount).mapToObj(i -> newProject("prk_key_" + i)).collect(toSet());
    User subscriber1 = newUser("subscriber1");
    User changeAuthor = newUser("changeAuthor");

    Set<ChangedIssue> fpIssues =
        projects.stream()
            .flatMap(
                project ->
                    streamOfIssues(
                        t ->
                            t.setProject(project)
                                .setNewIssueStatus(IssueStatus.FALSE_POSITIVE)
                                .setOldIssueStatus(IssueStatus.OPEN)
                                .setAssignee(subscriber1)))
            .collect(toSet());
    Set<ChangedIssue> wontFixIssues =
        projects.stream()
            .flatMap(
                project ->
                    streamOfIssues(
    ...
    IssuesChangesNotificationBuilder fpAndWontFixNotifications =
        new IssuesChangesNotificationBuilder(
            Stream.concat(fpIssues.stream(), wontFixIssues.stream()).collect(toSet()), userChange);
    when(emailNotificationChannel.isActivated()).thenReturn(true);
    projects.forEach(
        project ->
            when(notificationManager.findSubscribedEmailRecipients(
    ...
    when(emailNotificationChannel.deliverAll(anySet()))
        ...
        .thenThrow(new IllegalStateException("deliver should be called only once"));
    Set<IssuesChangesNotification> notifications =
        singleton(serializer.serialize(fpAndWontFixNotifications));
    reset(serializer);
    ...
    assertThat(requestsByRecipientEmail.get(emailOf(subscriber1.getLogin())))
        .containsOnly(
            new EmailDeliveryRequest(
                emailOf(subscriber1.getLogin()),
                new FPOrAcceptedNotification(userChange, wontFixIssues, FpPrAccepted.ACCEPTED)),
            new EmailDeliveryRequest(
                emailOf(subscriber1.getLogin()),
                new FPOrAcceptedNotification(userChange, fpIssues, FpPrAccepted.FP)));
  }
  ...
  private static EmailDeliveryRequest newEmailDeliveryRequest(
      IssuesChangesNotificationBuilder notif, User user, FpPrAccepted resolution) {
  ...
  private static Stream<ChangedIssue> streamOfIssues(Consumer<ChangedIssue.Builder> consumer) {
    return IntStream.range(0, 5)
        .mapToObj(
            i -> {
              ChangedIssue.Builder builder =
                  new ChangedIssue.Builder("key_" + i)
                      .setAssignee(
                          new User(randomAlphabetic(3), randomAlphabetic(4), randomAlphabetic(5)))
                      .setNewStatus(randomAlphabetic(12))
                      .setRule(newRandomNotAHotspotRule(randomAlphabetic(8)))
                      .setProject(
                          new Project.Builder(randomAlphabetic(9))
