...
public class AnticipatedTransitionTrackerTest {
  ...
  private final AnticipatedTransitionTracker<DefaultIssue, AnticipatedTransition> underTest =
      new AnticipatedTransitionTracker<>();
  ...
  public void givenIssuesAndAnticipatedTransitions_trackerShouldReturnTheExpectedMatching() {
    ...
    Tracking<DefaultIssue, AnticipatedTransition> tracking =
        underTest.track(issues, anticipatedTransitions);
    ...
    assertThat(matchedRaws.keySet())
        .containsExactlyInAnyOrder(
            issues.get(0), issues.get(1), issues.get(2), issues.get(3), issues.get(4));
    assertThat(unmatchedRaws).containsExactlyInAnyOrder(issues.get(5), issues.get(6));

    assertThat(matchedRaws)
        .containsEntry(issues.get(0), anticipatedTransitions.get(1))
  ...
  private List<DefaultIssue> getIssues() {
    return List.of(
        getIssue(
            1, "message1", "hash1", "rule:key1"), // should match transition 2 due to lvl 1 matching
        getIssue(
            2, "message2", "hash2", "rule:key2"), // should match transition 1 due to lvl 2 matching
        getIssue(
            3, "message3", "hash3", "rule:key3"), // should match transition 4 due to lvl 3 matching
        getIssue(
            4, "message4", "hash4", "rule:key4"), // should match transition 3 due to lvl 4 matching
        getIssue(
            5, "message5", "hash5", "rule:key5"), // should match transition 7 due to lvl 5 matching
  ...
  private AnticipatedTransition getAnticipatedTransition(
      Integer line, String message, String hash, String ruleKey) {
