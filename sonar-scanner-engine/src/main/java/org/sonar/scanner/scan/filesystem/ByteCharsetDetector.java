...
public class ByteCharsetDetector {
  ...
  public Charset detect(byte[] buf) {
    ...
    if (utf16.valid() == Validation.YES
        && validator.isValidUTF16(buf, UTF_16LE.equals(utf16.charset()))) {
    ...
    if (!UTF_8.equals(c)
        && (!isUtf16(c) || utf16.valid() == Validation.MAYBE)
        && validator.tryDecode(buf, c)) {
      return c;
    }
  ...
  private static boolean isUtf16Or32(Charset charset) {
    return isUtf16(charset)
        || MetadataGenerator.UTF_32BE.equals(charset)
        || MetadataGenerator.UTF_32LE.equals(charset);
  }
